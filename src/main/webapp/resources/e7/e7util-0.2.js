...
E7.isNumber = function(str) {
	if(str == undefined || str == null || isNaN(str) || (typeof str == 'string' && str.trim() == '')){
		return false;
	}
...
/***
 * 初始化select控件选项
 * @param selecter		select控件对象：$('#selecter')
 * @param items			选项列表:[{'id':'1', 'text':'北京'}, {'id':'2', 'text':'上海'}]
 * @param addEmpty		是否添加空元素
 * @param defaultValue	默认选项
 */
E7.initSelect = function(selecter, items, addEmpty, defaultValue) {
	$(selecter).empty();
	if(addEmpty) {
		$(selecter).append(new Option(E7.i18n.select, ''));
	}

	if(!$(selecter).data('items')) {
		$(selecter).data('items', items);
		$(selecter).data('valueField', 'id');
	}
	
	for (var i = 0; i < items.length; i++) {
		var op = new Option(items[i].text, items[i].id);
		if(defaultValue != undefined && defaultValue != null && defaultValue == items[i].id){
			op.selected = true;
		}
		$(selecter).append(op);
	}
 ...
 * @param defaultValue	默认选项
 */
E7.initSelect4NumberRange = function(selecter, first, last, addEmpty, defaultValue) {
	$(selecter).empty();
	if(addEmpty) {
		$(selecter).append(new Option(E7.i18n.select, ''));
	}
	for (var i = first; i <= last; i++) {
		...
		if(defaultValue != undefined && defaultValue != null && defaultValue == i){
			op.selected = true;
		}
		$(selecter).append(op);
	}
 ...
 * @param selecter		select控件对象选择器, 例如：'#selectId'
 * @param url			获取数据的url
 * @param nameField		名称对应的字段
 * @param valueField	值对应的字段
 * @param addEmpty		是否添加空元素
 * @param defaultValue	默认选项
 */
E7.initSelect4Field = function(selecter, items, nameField, valueField, addEmpty, defaultValue) {
	$(selecter).empty();
	if(addEmpty) {
		$(selecter).append(new Option(E7.i18n.select, ''));
	}
	...
	$(selecter).data('items', items);
	$(selecter).data('valueField', valueField);
	
	for (var i = 0; i < items.length; i++) {
		var op = new Option(items[i][nameField], items[i][valueField]);
		if(defaultValue != undefined && defaultValue != null && defaultValue == items[i].id){
			op.selected = true;
		}
		$(selecter).append(op);
 ...
 * @param callback 		回调函数
 */
E7.initSelectRemote = function(url, params, selecter, nameField, valueField, addEmpty, defaultValue, callback) {
	E7.get(url, params, function(result){
		var dataArr = [];
		if(result.data instanceof Array) {
			dataArr = result.data;
		}

		$(selecter).data('items', dataArr);
		$(selecter).data('valueField', valueField);
		
		var pairs = [];
		for(var i = 0; i < dataArr.length; i++) {
			var item = dataArr[i];
			pairs.push({'id':item[valueField], 'text':item[nameField]});
		}
		E7.initSelect(selecter, pairs, addEmpty, defaultValue);	
		
		if(typeof callback == "function"){
			callback(result, dataArr);
		}
	});
}

E7.getSelectedItem = function(selecter) {
	var items = $(selecter).data('items');
	var valueField = $(selecter).data('valueField');
	var value = $(selecter).val();
	
	if(value == '' || !items || !valueField) {
		return null;
	}
