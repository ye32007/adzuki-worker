(function($){
	...
	$.fn.e7tabs = function(options, param){
		...
		if (typeof options == 'string') {
			switch(options){
				...
				// 根据title获取tab列表
				case 'getTabsByTitle':
					return getTabsByTitle(target, param);
					
				case 'getActiveTab':
					return getActiveTab(target);

				case 'setActiveTab':
					return getActiveTab(target, param);
	...
	function addTab(target, tab){
		...
		for(var i=0; i<tabs.length; i++) {
			...
			} else if(tabs[i].title.trim() == tab.title.trim() && tabs[i].url.trim() == tab.url.trim()) {
				tabExist = true;
			}
		...
		//没有id，生成uuid
		if(tab.id == null) tab.id = Math.uuid();
	 ...
	 * 根据title获取tab列表
	 */
	function getTabsByTitle(target, title){
		var opts = $.data(target, 'e7tabs').options;
		var tabs = opts.tabs;
		var result = [];
		for(var i=0; i<tabs.length; i++) {
	...
	function setActiveTab(tab){
		...
		for(var i=0; i<tabs.length; i++) {
			...
			if(tab.title && tab.url) {
				if(tab.title == tabs[i].title && tab.url == tabs[i].url) {
					$('#' + target.id + ' .active').removeClass("active");
					$('#' + divContentId + ' .active').removeClass("active");
					$('#t_' + tabs[i].id).addClass("active");
					$('#c_' + tabs[i].id).addClass("active");
